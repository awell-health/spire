...
type columnMigration struct {
	...
	ddl    string // full ALTER TABLE clause, e.g. "ADD COLUMN phase VARCHAR(16) AFTER role"
...
var spireMigrations = []columnMigration{
	...
	{table: "agent_runs", column: "id", ddl: "ADD COLUMN id VARCHAR(32) NOT NULL PRIMARY KEY FIRST"},
	{table: "agent_runs", column: "bead_id", ddl: "ADD COLUMN bead_id VARCHAR(64) NOT NULL AFTER id", index: "CREATE INDEX idx_bead ON agent_runs (bead_id)"},
	{table: "agent_runs", column: "epic_id", ddl: "ADD COLUMN epic_id VARCHAR(64) AFTER bead_id", index: "CREATE INDEX idx_epic ON agent_runs (epic_id)"},
	{table: "agent_runs", column: "agent_name", ddl: "ADD COLUMN agent_name VARCHAR(128) AFTER epic_id"},
	{table: "agent_runs", column: "model", ddl: "ADD COLUMN model VARCHAR(64) NOT NULL AFTER agent_name", index: "CREATE INDEX idx_model ON agent_runs (model)"},
	{table: "agent_runs", column: "role", ddl: "ADD COLUMN role VARCHAR(16) NOT NULL AFTER model"},
	{table: "agent_runs", column: "phase", ddl: "ADD COLUMN phase VARCHAR(16) AFTER role", index: "CREATE INDEX idx_phase ON agent_runs (phase)"},
	{table: "agent_runs", column: "context_tokens_in", ddl: "ADD COLUMN context_tokens_in INT AFTER phase"},
	{table: "agent_runs", column: "context_tokens_out", ddl: "ADD COLUMN context_tokens_out INT AFTER context_tokens_in"},
	{table: "agent_runs", column: "total_tokens", ddl: "ADD COLUMN total_tokens INT AFTER context_tokens_out"},
	{table: "agent_runs", column: "turns", ddl: "ADD COLUMN turns INT AFTER total_tokens"},
	{table: "agent_runs", column: "duration_seconds", ddl: "ADD COLUMN duration_seconds INT AFTER turns"},
	{table: "agent_runs", column: "startup_seconds", ddl: "ADD COLUMN startup_seconds INT AFTER duration_seconds"},
	{table: "agent_runs", column: "working_seconds", ddl: "ADD COLUMN working_seconds INT AFTER startup_seconds"},
	{table: "agent_runs", column: "queue_seconds", ddl: "ADD COLUMN queue_seconds INT AFTER working_seconds"},
	{table: "agent_runs", column: "review_seconds", ddl: "ADD COLUMN review_seconds INT AFTER queue_seconds"},
	{table: "agent_runs", column: "result", ddl: "ADD COLUMN result VARCHAR(32) NOT NULL AFTER review_seconds", index: "CREATE INDEX idx_result ON agent_runs (result)"},
	{table: "agent_runs", column: "review_rounds", ddl: "ADD COLUMN review_rounds INT DEFAULT 0 AFTER result"},
	{table: "agent_runs", column: "artificer_verdict", ddl: "ADD COLUMN artificer_verdict VARCHAR(32) AFTER review_rounds"},
	{table: "agent_runs", column: "review_step", ddl: "ADD COLUMN review_step VARCHAR(16) AFTER artificer_verdict"},
	{table: "agent_runs", column: "review_round", ddl: "ADD COLUMN review_round INT AFTER review_step"},
	{table: "agent_runs", column: "spec_file", ddl: "ADD COLUMN spec_file VARCHAR(256) AFTER review_round"},
	{table: "agent_runs", column: "spec_size_tokens", ddl: "ADD COLUMN spec_size_tokens INT AFTER spec_file"},
	{table: "agent_runs", column: "focus_context_tokens", ddl: "ADD COLUMN focus_context_tokens INT AFTER spec_size_tokens"},
	{table: "agent_runs", column: "files_changed", ddl: "ADD COLUMN files_changed INT AFTER focus_context_tokens"},
	{table: "agent_runs", column: "lines_added", ddl: "ADD COLUMN lines_added INT AFTER files_changed"},
	{table: "agent_runs", column: "lines_removed", ddl: "ADD COLUMN lines_removed INT AFTER lines_added"},
	{table: "agent_runs", column: "tests_added", ddl: "ADD COLUMN tests_added INT AFTER lines_removed"},
	{table: "agent_runs", column: "tests_passed", ddl: "ADD COLUMN tests_passed BOOLEAN AFTER tests_added"},
	{table: "agent_runs", column: "system_prompt_hash", ddl: "ADD COLUMN system_prompt_hash VARCHAR(64) AFTER tests_passed"},
	{table: "agent_runs", column: "golden_run", ddl: "ADD COLUMN golden_run BOOLEAN DEFAULT FALSE AFTER system_prompt_hash", index: "CREATE INDEX idx_golden ON agent_runs (golden_run)"},
	{table: "agent_runs", column: "cost_usd", ddl: "ADD COLUMN cost_usd DECIMAL(10,4) AFTER golden_run"},
	{table: "agent_runs", column: "started_at", ddl: "ADD COLUMN started_at DATETIME NOT NULL AFTER cost_usd"},
	{table: "agent_runs", column: "completed_at", ddl: "ADD COLUMN completed_at DATETIME AFTER started_at"},

	// --- golden_prompts columns (in table order) ---
	{table: "golden_prompts", column: "run_id", ddl: "ADD COLUMN run_id VARCHAR(32) NOT NULL PRIMARY KEY FIRST"},
	{table: "golden_prompts", column: "bead_id", ddl: "ADD COLUMN bead_id VARCHAR(64) NOT NULL AFTER run_id"},
	{table: "golden_prompts", column: "system_prompt", ddl: "ADD COLUMN system_prompt TEXT AFTER bead_id"},
	{table: "golden_prompts", column: "spec_excerpt", ddl: "ADD COLUMN spec_excerpt TEXT AFTER system_prompt"},
	{table: "golden_prompts", column: "focus_context", ddl: "ADD COLUMN focus_context TEXT AFTER spec_excerpt"},
	{table: "golden_prompts", column: "tags", ddl: "ADD COLUMN tags JSON AFTER focus_context"},
	{table: "golden_prompts", column: "context_tokens", ddl: "ADD COLUMN context_tokens INT AFTER tags"},
}
