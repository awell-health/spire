...
func cmdInit(args []string) error {
	...
	if role == "satellite" {
		...
		if _, statErr := os.Stat(localBeads); statErr == nil {
			...
			// No local .beads — initialize. Use --database to connect to an
			// existing server database without generating a new project ID.
			// Falls back to --force if --database fails (fresh cluster).
			_, initErr := bd("init", "--database", prefix, "--prefix", prefix)
			if initErr != nil {
				_, initErr = bd("init", "--force", "--prefix", prefix)
			}
			if initErr != nil {
				return fmt.Errorf("bd init failed: %w\n  Try: bd init --database %s --prefix %s", initErr, prefix, prefix)
			}
	...
	// --- Gitignore machine-specific files ---
	ensureGitignore(cwd)
...
// ensureGitignore adds machine-specific entries to the repo's .gitignore.
// Only adds entries that aren't already present. Creates the file if missing.
func ensureGitignore(repoPath string) {
	...
	if err != nil {
		fmt.Printf("  Warning: could not update .gitignore: %s\n", err)
		return
	}
	defer f.Close()
