...
func TestSummonLocal_TransitionsOpenToInProgress(t *testing.T) {
	...
	summonBeginWorkFunc = func(deps lifecycle.Deps, _ wizardregistry.Registry, beadID string, opts lifecycle.BeginOpts) (string, error) {
		gotBeadID = beadID
		return "att-stub", nil
	}
	summonSpawnFunc = stubSpawn
...
func TestSummonLocal_AuthFailureSkipsBeginWork(t *testing.T) {
	...
	beginCalls := 0
	summonBeginWorkFunc = func(_ lifecycle.Deps, _ wizardregistry.Registry, _ string, _ lifecycle.BeginOpts) (string, error) {
		beginCalls++
