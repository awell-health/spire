...
func checkAndMarkRelapse(sourceBeadID, failureClass string, deps *Deps) {
	...
	reusable := true
	learnings, err := store.ListClosedRecoveryBeads(store.RecoveryLookupFilter{
	...
	for _, l := range learnings {
		...
		_ = store.SetBeadMetadataMap(l.BeadID, map[string]string{
			recovery.KeyOutcome: "relapsed",
		})

		// Update the SQL table row as well.
		if sqlErr := store.UpdateLearningOutcomeAuto(l.BeadID, "relapsed"); sqlErr != nil {
			fmt.Fprintf(os.Stderr, "warning: relapse update SQL for %s: %s\n", l.BeadID, sqlErr)
		}

		// Add comment to the old recovery bead.
		if deps != nil && deps.AddComment != nil {
			_ = deps.AddComment(l.BeadID, fmt.Sprintf(
